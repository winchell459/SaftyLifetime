...
namespace Examples
{
    public class myVictorsFPS : MonoBehaviour
    {
        bool binded;
        ...
        void Update()
        {
            ...
            {
                Crouched = true;
                GetComponent<myPlayerController>().Crouch(true);
            }

            else //(TCKInput.GetAction("crouchBtn", EActionEvent.Up))
            {
                Crouched = false;
                GetComponent<myPlayerController>().Crouch(false);
            }

            if (TCKInput.GetAction("pickupBtn", EActionEvent.Press))
            {
                GetComponent<myPlayerController>().SetIsPickingUpItem(true);
            }
            if (TCKInput.GetAction("pickupBtn", EActionEvent.Up))
            {
                GetComponent<myPlayerController>().SetIsPickingUpItem(false);
            }
