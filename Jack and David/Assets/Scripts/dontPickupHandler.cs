...
public class dontPickupHandler : MonoBehaviour
{
    ...
    void Update()
    {
        if (triggered){
            ...
            foreach (Item item in PickupItems)
            {
                ...
                itemsNeeded.Add(newItem);
                newItem.GetComponent<dontPickupItem>().myPH = this;
                triggered = false;
                counter += 1;
            }
        }
    ...
    private void addItem(GameObject myItem){
        ...
        if (itemsNeeded.Count >= DontPickupLimit)
        {
